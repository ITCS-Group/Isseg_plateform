...
| `BIBLIOTHECAIRE` | Bibliothèque (catalogue, emprunts, abonnés) | ✅ Backend livré (`feature/bibliotheque`, 19-20/08 ; restriction emprunt domicile le 20/08) : `SectionBibliotheque`/`Ouvrage`/`Emprunt`/`Abonne`/`DocumentAcademique`/`Reservation`, endpoints `/ouvrages`, `/emprunts`(+`/retour`), `/abonnes`, `/bibliotheque/stats/dashboard`. **Le prêt à domicile (`POST /emprunts`) n'est autorisé que pour un emprunteur de type `ENSEIGNANT`** (config `BIBLIOTHEQUE_EMPRUNT_DOMICILE_TYPES_AUTORISES`, décision métier du 05/08/2026 — voir CLAUDE.md § Library Loans). Aucun écran frontend. |
